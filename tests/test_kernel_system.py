...
from archinstall_zfs.kernel.package_manager import ZFSPackageManager, InstallationResult
from archinstall_zfs.kernel.fallback import FallbackStrategy, EnhancedZFSInstaller
...
class TestFallbackStrategy:
    ...
    def test_should_attempt_precompiled(self):
        ...
        assert FallbackStrategy.should_attempt_precompiled(
            variant_with_precompiled, ZFSModuleMode.PRECOMPILED
        ) is True
        
        # Should not attempt precompiled (not supported)
        assert FallbackStrategy.should_attempt_precompiled(
            variant_without_precompiled, ZFSModuleMode.PRECOMPILED
        ) is False
        
        # Should not attempt precompiled (DKMS requested)
        assert FallbackStrategy.should_attempt_precompiled(
            variant_with_precompiled, ZFSModuleMode.DKMS
        ) is False
...
class TestZFSPackageManager:
    ...
    def test_install_precompiled_success(self, mock_syscmd):
    ...
    @patch('archinstall_zfs.kernel.package_manager.SysCommand')
    def test_install_with_fallback(self, mock_syscmd):
        ...
        with patch.object(self.package_manager, '_try_precompiled_install', return_value=False):
            with patch.object(self.package_manager, '_install_dkms', return_value=True):
                result = self.package_manager.install_zfs_packages(
                    "linux-lts",
                    ZFSModuleMode.PRECOMPILED,
                    mock_installation
