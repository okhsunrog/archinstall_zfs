...
class FallbackStrategy:
    ...
    @staticmethod
    def get_fallback_chain(variant: KernelVariant, requested_mode: ZFSModuleMode) -> list[tuple[KernelVariant, ZFSModuleMode]]:
    ...
    def should_attempt_precompiled(variant: KernelVariant, requested_mode: ZFSModuleMode) -> bool:
        ...
        """
        return requested_mode == ZFSModuleMode.PRECOMPILED and variant.supports_precompiled and variant.zfs_precompiled_package is not None
...
class EnhancedZFSInstaller:
    ...
    def install_with_fallback(self, kernel_name: str, preferred_mode: ZFSModuleMode, installation: Any = None) -> InstallationResult:
        ...
        for attempt_num, (attempt_variant, attempt_mode) in enumerate(fallback_chain, 1):
            ...
            result = self.package_manager.install_zfs_packages(attempt_variant.name, attempt_mode, installation)
        ...
        return last_result or InstallationResult(kernel_variant=variant, requested_mode=preferred_mode, success=False)
    ...
    def get_installation_summary(self, kernel_name: str, preferred_mode: ZFSModuleMode) -> str:
        ...
        summary_lines = [f"Installation plan for {variant.display_name}:", f"Requested mode: {preferred_mode.value}"]

        for attempt_num, (attempt_variant, attempt_mode) in enumerate(fallback_chain, 1):
            packages = attempt_variant.get_precompiled_packages() if attempt_mode == ZFSModuleMode.PRECOMPILED else attempt_variant.get_dkms_packages()

            mode_text = "primary" if attempt_num == 1 else "fallback"
            summary_lines.append(f"  {mode_text.capitalize()}: {attempt_mode.value} - {', '.join(packages)}")
    ...
    def validate_installation_plan(self, kernel_name: str, preferred_mode: ZFSModuleMode) -> list[str]:
        ...
        if preferred_mode == ZFSModuleMode.PRECOMPILED and not variant.supports_precompiled:
            errors.append(f"Kernel {variant.name} does not support precompiled ZFS modules. DKMS will be used instead.")
        ...
        try:
            ...
            if not self.package_manager.verify_packages_available(dkms_packages):
                errors.append(f"Required DKMS packages not available: {', '.join(dkms_packages)}")
