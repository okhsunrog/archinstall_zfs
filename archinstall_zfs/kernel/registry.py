...
class KernelRegistry:
    ...
    def auto_detect_variants(self) -> None:
        ...
        for kernel_name in potential_kernels:
            if kernel_name not in self._variants:
                if self._check_package_exists(kernel_name):
    ...
    def _load_default_variants(self) -> None:
        ...
        self.register_variant(KernelVariant(
            name="linux-lts",
    ...
    def save_to_file(self, config_path: Path) -> None:
        ...
        try:
            import json
            config_data = {
                "kernel_variants": [v.to_dict() for v in self._variants.values()]
            }
...
def get_kernel_registry() -> KernelRegistry:
    ...
    if _global_registry is None:
        ...
        config_paths = [
            Path("/etc/archinstall-zfs/kernel-variants.json"),
            Path.home() / ".config" / "archinstall-zfs" / "kernel-variants.json"
        ]
